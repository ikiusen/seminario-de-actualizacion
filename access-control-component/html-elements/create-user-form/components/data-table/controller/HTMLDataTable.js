...
class HTMLDataTable extends HTMLElement
{
	...
	{
		let selectedrows = [];
		for (let current of this.view.table.tBodies[0].childNodes )
		{
			console.log(current.cells[0]);
			if(current.cells[0].childNodes[0].checked)
			{
				console.log(current);
				selectedrows.push(current.cells[1].childNodes[0].innerText);
			}
		}
		return selectedrows;
	}
